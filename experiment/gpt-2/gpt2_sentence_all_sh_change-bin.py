...
# load tokenizer
#tokenizer = T5Tokenizer.from_pretrained('rinna/imi-xsmall/'+model_dir)
...
for ram in range(3, len(sys.argv)):
    path = sys.argv[ram]
    print(path)
    probs = eval_test(tokenizer, model, path)
    path = path.replace('.','/').split('/')
    path = path[-2]
    print(path)
    save_file_name = './probs_all_margin'+sys.argv[2]+'_'+path+'.json'
